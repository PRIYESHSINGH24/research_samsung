...
Paper Settings:
  - LeNet-5 Teacher / LeNet-5-Half Student
  - 48,000 DIs (4,800 per class!)
  - β={1.0, 0.1}, τ=20
  - Same as MNIST but different dataset + more DIs
"""
import torch
...
def get_loaders():
    transform = get_fmnist_transform()
    ...
    print(f"  FMNIST Train: {len(train_data):,} | Test: {len(test_data):,}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("  TABLE 2 — FASHION-MNIST (Paper Exact Settings)")
    print("  48,000 DIs (4,800 per class)")
    print("="*60)
    ...
    for epoch in range(1, 101):
        ...
        if acc > best_teacher:
            best_teacher = acc
            torch.save(teacher.state_dict(), f'{config.CHECKPOINT_DIR}fmnist_teacher.pth')
        if epoch % 10 == 0:
            print(f"  Epoch {epoch}: {acc:.2f}% | Best: {best_teacher:.2f}%")
    
    print(f"  Teacher-CE: {best_teacher:.2f}% (Paper: 90.84%)")
    ...
    print(f"  Student-CE: {best_sce:.2f}% (Paper: 89.43%)")
    ...
    print(f"  Student-KD: {best_skd:.2f}% (Paper: 89.66%)")
    
    # ── 4. Generate 48,000 DIs ──
    print("\n  --- Generate 48,000 DIs (4,800 per class) ---")
    teacher.eval()
    ...
    NUM_PER_CLASS = 4800  # Paper: 48K total!
    
    for cls in range(10):
        alpha = sim[cls]
        class_dis = []
        for beta in [1.0, 0.1]:
            n_per_beta = NUM_PER_CLASS // 2  # 2400 each
    ...
    print(f"  ZSKD: {best_zskd:.2f}% (Paper: 79.62%)")
    ...
    print(f"  {'-'*43}")
    print(f"  {'Teacher-CE':<25s} {f'{best_teacher:.2f}%':>8s}  {'90.84%':>8s}")
    print(f"  {'Student-CE':<25s} {f'{best_sce:.2f}%':>8s}  {'89.43%':>8s}")
    print(f"  {'Student-KD':<25s} {f'{best_skd:.2f}%':>8s}  {'89.66%':>8s}")
    print(f"  {'ZSKD (48K DIs)':<25s} {f'{best_zskd:.2f}%':>8s}  {'79.62%':>8s}")
    print(f"{'='*60}")